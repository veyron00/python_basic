...
def payment_calculation2(s, n, i,):
    i = i / 100
    remaining_debt = s
    K = (i * (1 + i) ** n) / ((1 + i) ** n - 1)
    annuity_payment = round(K * remaining_debt, 2)
    for period in range(1, n + 1):
        interest_paid = i * remaining_debt
        loan_body_paid = annuity_payment - interest_paid
        print(f'\nПериод: {period}')
        print('\nОстаток долга на начало периода:', remaining_debt)
        print('\nВыплачено процентов:', interest_paid)
        print("\nВыплачено тела кредита:", loan_body_paid)
        n -= 1
        remaining_debt -= loan_body_paid
    print(f'\n\nОстаток долга: {remaining_debt}')


def payment_calculation1(s, n, i):
  i = i / 100
  remaining_debt = s
  K = (i * (1 + i) ** n) / ((1 + i) ** n - 1)
  annuity_payment = round(K * remaining_debt, 2)
  for period in range(1, n + 1):
      if period <= 3:
          ...
          return payment_calculation2(s, n, i)
    

loan_amount = float(input('\nВведите сумму кредита: '))
loan_term = int(input('\nНа сколько лет выдан? '))
interest_rate = float(input('\nСколько процентов годовых? '))

payment_calculation1(loan_amount, loan_term, interest_rate)