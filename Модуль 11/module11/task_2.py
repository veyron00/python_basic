...
number_attempts = int(input('Введите количество чисел: '))
for num in range(number_attempts):
    number = float(input('Введите число: '))
    if number >= 0:
        x = math.ceil(number)
        print(f'Натуральный логарифм от введенного числа: {math.log(x)}')
    else:
        x = math.floor(number)
        print(f'Экспонента в степени от введенного числа: {math.exp(x)}')
